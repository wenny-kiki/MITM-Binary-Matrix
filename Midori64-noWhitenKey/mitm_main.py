...
class constraints_generator:
    ...
    def genConstraints_additional(self):
        ...
        self.m.addConstr(quicksum(self.d1) - quicksum(self.CD_MC_Blue_Real + self.CD_ARK_Blue + self.CD_Xor_Blue) == d1_CD)
        self.m.addConstr(quicksum(self.d2) - quicksum(self.CD_MC_Red_Real + self.CD_ARK_Red + self.CD_Xor_Red) == d2_CD)
        self.m.addConstr(d1_CD + d2_CD == 0)
        self.m.addConstr(quicksum(self.d1_Key1 + self.d1_Key2) - quicksum(self.CD_Key_MC_Blue_Real) == self.GDeg1)
        self.m.addConstr(quicksum(self.d2_Key1 + self.d2_Key2) - quicksum(self.CD_Key_MC_Red_Real) == self.GDeg2)
        self.m.addConstr(self.Obj <= self.GDeg1)
        self.m.addConstr(self.Obj <= self.GDeg2)
        if self.ini_r > self.mat_r:
            for bi in range(bs):
                self.m.addGenConstrAnd(self.ExistGray[bi], [self.IP_1[self.TR][bi], self.IP_2[self.TR][bi]])
                self.m.addConstr(self.IP_1[0][bi] == 0)
                self.m.addConstr(self.IP_2[0][bi] == 1)
                self.m.addConstr(self.IP_2[self.TR][bi] == 1)
        if self.ini_r < self.mat_r:
            for bi in range(bs):
                self.m.addGenConstrAnd(self.ExistGray[bi], [self.IP_1[0][bi], self.IP_2[0][bi]])
                self.m.addConstr(self.IP_1[self.TR][bi] == 0)
                self.m.addConstr(self.IP_2[self.TR][bi] == 1)
                self.m.addConstr(self.IP_2[0][bi] == 1)
        self.m.addConstr(quicksum(self.ExistGray) >= 1)
    ...
    def gen_Model(self, LPFile, SolFile):
        ...
        self.m.write(LPFile)
        self.m.setParam('TimeLimit', 200 * 60)
        self.m.optimize()
